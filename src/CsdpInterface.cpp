...
void CsdpInterface::print(std::ostream & stream ) const {
...
size_t _n(0);
  ...
  for (auto const & c : _cTerms) {
    C.blocks[1].data.mat[my_ijtok(c.first + 1, c.first + 1, _n + 1)] = c.second;
  }

  for (auto const & q : _qTerms) {
    C.blocks[1].data.mat[my_ijtok(q.first.first + 1, q.first.second + 1, _n + 1)] =
        q.second;
    C.blocks[1].data.mat[my_ijtok(q.first.second + 1, q.first.first + 1, _n + 1)] =
        q.second;
  }
...
//		std::cout << blockptr->numentries << std::endl;
  ...
  ///
  std::string const probName("qp_sdp.dat-s");
  write_prob(const_cast<char*>(probName.c_str()), _n + 1, _n + 1, C, b,
             constraints);
  ///
  ...
  if (ret == 0)
    ...
    printf("SDP failed.\n");
  std::string const solName("qp_sdp.sol");
  write_sol(const_cast<char*>(solName.c_str()), _n + 1, _n + 1, X, y, Z);
